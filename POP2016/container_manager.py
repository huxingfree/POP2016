...
def run(ptype, path, node=None, port=None, memory=None):
    global MANAGER
    constraint=''
    if node is not None:
        constraint=" -e constraint:node_name==node%d" % (int(node))
    if port is None:
        port=get_valid_port(10001, 18999)
    else:
        port=int(port)
    memlimit=''
    ...
    if ptype=='php':
        code, output=commands.getstatusoutput("docker %s run -id%s -p %d:80 -p %d:22 -v /root/data/repo/php%s:/var/www/html/%s pop2016/php" % (MANAGER, memlimit, port, port+10000, path, constraint))
    elif ptype=='python':
        code, output=commands.getstatusoutput("docker %s run -id%s -p %d:80 -p %d:22 -v /root/data/repo/python%s:/home/%s pop2016/python" % (MANAGER, memlimit, port, port+10000, path, constraint))
    elif ptype=='javaweb':
        code, output=commands.getstatusoutput("docker %s run -id%s -p %d:8080 -p %d:22 -v /root/data/repo/javaweb/%s:/usr/local/tomcat/webapps/ROOT/%s pop2016/tomcat" % (MANAGER, memlimit, port, port+10000, path, constraint))
    else:
    ...
    if code is 0:
        ...
        for domain in DOMAINS:
            if domain in output:
                response = {'code': '0', 'domain': domain, 'port': "%d" % (port), 'sshport': "%d" % (port+10000), 'dockerid': dockerid}
...
def startservice(ptype, path=None, node=None, port=None, memory=None):
    global MANAGER
    if node is None:
        node=1
    constraint=" -e constraint:node_name==node%d" % (int(node))
    if port is None:
        port=get_valid_port(9001, 9999)
    else:
        port=int(port)
    memlimit=''
    if memory is not None:
    ...
    if ptype=='tomcat':
        code, output=commands.getstatusoutput("docker %s run -id%s -p %d:8080 -p %d:22 -v /root/data/repo/:/data/ -v /root/data/service%s:/usr/local/tomcat/webapps/ROOT/%s pop2016/tomcat-server" % (MANAGER, memlimit, port, port+30000, path, constraint))
    elif ptype=='gateone':
        code, output=commands.getstatusoutput("docker %s run -id%s -p %d:8000 -p %d:22%s pop2016/gateone" % (MANAGER, memlimit, port, port+30000, constraint))
    else:
    ...
    if code is 0:
        ...
        for domain in DOMAINS:
            if domain in output:
                response = {'code': '0', 'domain': domain, 'port': "%d" % (port), 'sshport': "%d" % (port+30000), 'dockerid': dockerid}
                return obj_to_json(response)
        return reply(6, "Unable to find the domain")
    else:
        return reply(5, output)
...
def main():
    ...
    if action=='run':
        return run(ptype, path, node, port, memory)
    if action=='startservice':
        return startservice(ptype, path, node, port, memory)
    if action=='stat':
